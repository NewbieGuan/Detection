...
CCameraImage::CCameraImage()
{
	...
	gapValue = frameCount / (sampleCountStrip + 1);    // 间隔
...
void CCameraImage::TransferData(unsigned char *pData, int nWidth, int nHeight)
{
	...
	//if (sampleCountStrip == frameCount)
	//{
	//	// 保存图像路径
	//	CString CstrPath;
	//	CString CstrIndex;
	...
	//if ((sampleCountStrip == frameCount) && (saveIndex <= sampleCountStrip))
	//{
	//	// 保存图像路径
	//	CString CstrPath;
	//	CString CstrIndex;
	...
	//else
	//{
	//	gapValue = frameCount / sampleCountStrip;
	//	int mod = saveIndex % gapValue;
	//	int n = saveIndex / gapValue;

	//	if ((mod == 0) && (n <= sampleCountStrip))
	//	{
	//		// 保存图像路径
	//		CString CstrPath;
	//		CString CstrIndex;
	//		CstrIndex.Format(_T("\\%d-%d"), stripIndex, n);
	//		CstrPath = childFolderCStr + CstrIndex;
	//		CstrPath += ".bmp";
...
//void CCameraImage::ComputeAccuracy(Mat& srcImg, int i, int j)
//{
//	Mat dstImg;
...
//
//		//3.29晚
//		//if (minRadius >= m_DiaThreshold && length < 2000)
//		//{
...
//		//	}
//
//		//	int ratio = maxLength / minRadius;  //利用长宽比来滤除条状纹理
//		//	if (vecArea.size() < 10000 && ratio < 8)
//		//	{
//		//		// 最大粒径
//		//		maxRadius = maxLength * m_resolution;
//
//		//		valueCount++;
//		//		areaSum += area;
//		//		lengthSum += length;
...
//		//		cv::drawContours(dstImg2, contours, index, color, CV_FILLED, 8, hierarchy);
//		//	}
//		//}
//		if (minRadius >= m_DiaThreshold)
//		{
//			if (length * m_resolution > 30000)  //阈值30000微米
//			{
//				cv::drawContours(dstImg2, contours, index, Scalar(255, 255, 255), CV_FILLED, 8, hierarchy);
//				areaStrip -= area * m_resolution*m_resolution;
//			}
//			else if (bigL / smallL > 5)
//			{
//				cv::drawContours(dstImg2, contours, index, Scalar(0, 255, 0), CV_FILLED, 8, hierarchy);
//				areaStrip -= area * m_resolution*m_resolution;
//			}
//			else if (vecArea.size() < 1000)
//			{
//				double maxLength = 0.0;
//				double curLenth = 0.0;
//				double fValue1 = 0, fValue2 = 0;
...
//	// 保存结果图像
//	string strPathColor;
//	strPathColor = childFolderStr + "\\" + to_string(i) + "-" + to_string(j);
//	strPathColor += "-color.BMP";
...
//  areaStrip = areaStrip * m_resolution * m_resolution;          // 一小条的像素面积
