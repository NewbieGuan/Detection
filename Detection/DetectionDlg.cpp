...
void CDetectionDlg::OnBnClickedBtnStartDetection()
{
	...
	m_sDistanceCamera = (double)_ttof(dlgMotionPara.m_sSetDistanceCamera);
	...
	/*
	int m_mode = ((int)(stepCamera / 2048))*2048 + 2048 - stepCamera;
	if (m_mode < 20)
		stepCamera = stepCamera + (20 - m_mode);
	else if (m_mode > 160)
		stepCamera = stepCamera + (m_mode - 160);
	*/

	// stepPlatform = (m_sDistancePlatform)*m_steps_mm;
	//double m = 2.8;  // 重叠量
	//double d = 48.0;  // 每次运动距离
	...
	if (stepCamera > 0)
	{
		//frameCountDlg = (int)(((stepCamera / 4) + heightnew - 1) / heightnew);    // 两倍频
		frameCountDlg = (int)((stepCamera / 4) / heightnew);    // 两倍频
	}
	else
	{
		frameCountDlg = (int)((-stepCamera / 4) / heightnew);    // 两倍频
	...
	else
	{
		...
		CString subStr;
		// subStr = _T("Camera is not connected, config file is not loaded or motion parameters are not set!\r\n");
